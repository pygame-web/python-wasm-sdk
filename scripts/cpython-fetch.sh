...
then
    wget -c https://www.python.org/ftp/python/3.14.3/Python-3.14.3.tar.xz
    tar xf Python-3.14.3.tar.xz
    ln -s Python-3.14.3 cpython${PYBUILD}
    ...
    pushd cpython${PYBUILD}
        ...
        cat ${SDKROOT}/patches/Python-3.14.3/*diff | patch -p1
    popd
...
then
    ...
    then
        ...
        wget -q -c  https://www.python.org/ftp/python/3.13.12/Python-3.13.12.tar.xz
        tar xf Python-3.13.12.tar.xz || exit 90
        ln -s Python-3.13.12 cpython${PYBUILD}
...
then
    ...
    # patch host to be PIC // now done in sdk
